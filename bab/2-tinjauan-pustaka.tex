...
\subsection{Perbandingan model YOLO}

Pada bagian ini, akan dibahas perbandingan antara model sebelumnya. Ketiga model ini merupakan varian dari algoritma YOLO (\emph{You Only Look Once}) yang terkenal dalam bidang deteksi objek. Masing-masing model memiliki keunggulan dan kekurangan tersendiri dalam hal kecepatan, akurasi, dan efisiensi. Perbandingan ini bertujuan untuk memberikan gambaran yang jelas mengenai performa dari setiap model dalam berbagai aspek, sehingga dapat membantu dalam memilih model yang paling sesuai untuk aplikasi tertentu.

\begin{table}[H]
  \centering
  \caption{Comparison of YOLOv8, YOLOv10, and YOLOv11 models}
  \label{tab:Comparison}
  \begin{tabular}{|l|l|c|c|c|c|}
  \hline
  \multicolumn{1}{|l|}{YOLO models} & \multicolumn{1}{l|}{YOLO}          & \multicolumn{1}{l|}{Preprocessing Speed} & \multicolumn{1}{l|}{Inference Speed} & \multicolumn{1}{l|}{Speed (ms)} \\
  \multicolumn{1}{|l|}{}            & \multicolumn{1}{l|}{Configuration} & \multicolumn{1}{l|}{(ms)}                & \multicolumn{1}{l|}{(ms)}            & \multicolumn{1}{l|}{}           \\ \hline
  ...
  & YOLOv8x & \textbf{0.9} & 24.8 & 2.3 \\ \cline{1-5}

  \multirow{5}{*}{YOLOv10}
  & YOLOv10n & 1.4 & \textbf{5.5} & 1.6 \\
  & YOLOv10s & 1.3 & 7.7 & 1.6 \\
  & YOLOv10m & 1.3 & 13.0 & 1.6 \\
  & YOLOv10b & 1.3 & 16.7 & 1.5 \\
  & YOLOv10x & 1.2 & 26.5 & 1.5 \\ \cline{1-5}

  \multirow{5}{*}{YOLOv11}
  & YOLOv11n & \textbf{0.1} & \textbf{2.4} & 2.2 \\
  & YOLOv11s & \textbf{0.1} & 5.0 & 2.5 \\
  & YOLOv11m & \textbf{0.1} & 11.9 & \textbf{0.6} \\
  & YOLOv11l & 0.2 & 14.6 & 1.2 \\
  & YOLOv11x & 0.6 & 26.3 & \textbf{0.6} \\ \cline{1-5}

\end{tabular}
\end{table}

Dari tabel \ref{tab:Comparison}, dapat dilihat bahwa model YOLOv11 memiliki keunggulan dalam hal kecepatan preprocessing dan inference dibandingkan dengan YOLOv8 dan YOLOv10.
Prapemrosesan mengacu pada tahap awal di mana gambar disiapkan untuk analisis. Proses ini melibatkan berbagai penyesuaian seperti skala, normalisasi, dan augmentasi untuk mengoptimalkan gambar agar sesuai untuk deteksi objek. Penyesuaian skala memastikan bahwa gambar berada dalam resolusi yang tepat untuk analisis, sementara normalisasi membantu dalam mengurangi variasi intensitas piksel yang tidak diinginkan. Augmentasi data, seperti rotasi, pemotongan, dan perubahan warna, digunakan untuk meningkatkan keragaman dataset, yang pada gilirannya membantu model dalam mengenali objek dalam berbagai kondisi dan sudut pandang.

Inferensi mengacu pada fase inti dari proses di mana model menganalisis gambar yang telah dipraproses untuk mendeteksi dan mengidentifikasi objek berdasarkan fitur dan pola yang telah dipelajari selama pelatihan. Setelah inferensi, pascapemrosesan diterapkan untuk menyempurnakan keluaran dari model. Teknik seperti Non-Maximum Suppression (NMS) digunakan untuk menghilangkan deteksi yang berlebihan dengan mempertahankan hanya deteksi dengan skor tertinggi dan menghapus yang tumpang tindih. Tahap ini memfinalisasi daftar objek yang terdeteksi, memastikan bahwa hasil akhir yang disajikan adalah yang paling akurat dan relevan\cite{sapkota2024}.
